package com.example.crm.repository;

import org.springframework.data.domain.PageRequest;
import org.springframework.data.mongodb.repository.Query;
import org.springframework.data.mongodb.repository.ReactiveMongoRepository;

import com.example.crm.domain.CustomerDocument;

import reactor.core.publisher.Flux;

public interface CustomerDocumentReactiveRepository extends ReactiveMongoRepository<CustomerDocument, String>{

	Flux<CustomerDocument> findAllByBirthYearBetween(int from, int year);
	
	@Query("{'birthYear': {'$gt': ?0, '$lt': ?1}}") // native query
	Flux<CustomerDocument> bul(int from, int to);

	@Query("{}")
	Flux<CustomerDocument> findAllFlux(PageRequest page);
	
}
